...
def calculate_sha1_time(data):
    ...
    sha1.update(data)
    sha1.hexdigest()#hash value
    end_time = time.time()
    return end_time - start_time

def test_sha1():
    total_time = 0
    random_data = os.urandom(320)
    for _ in range(TEST_TIMES):
        total_time += calculate_sha1_time(random_data)
    return total_time
...
def test_puf(num_challenges, puf_type, **kwargs):
    puf = XORArbiterPUF(n=128, k=1, seed=1)
    challenges = random_inputs(n=puf.n, N=num_challenges, seed=1)
    # print(challenges)
    total_time = 0
    for _ in range(TEST_TIMES):
        start_time = time.time()
...
def test_reproduce_key_time():
    ...
    extractor = FuzzyExtractor(16, 1)

    # Original input value (e.g., biometric data)
    original_value = b'AABBCCDDEEFFGGHH'
    close_value = b'AABBCCDDEEFFGGHI'  # A value close to the original one
...
if __name__ == "__main__":
    ...
    total_puf_time = test_puf(1, XORArbiterPUF, **puf_params)
